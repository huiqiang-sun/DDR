...
def load_image(imfile, h=None, w=None):
    long_dim = 768
...
def load_image_list(image_files):
    ...
    if w >= h:
        for imfile in sorted(image_files):
            images.append(load_image(imfile, h=h))
    else:
        for imfile in sorted(image_files):
            images.append(load_image(imfile, w=w))
...
def run_optical_flows(args):
    ...
    with torch.no_grad():
        ...
        for i in range(images.shape[0]-1):
            ...
            if VIZ:
                viz_flow_path = './viz_flow/' + data_name
                if not os.path.exists(viz_flow_path):
                    os.makedirs(viz_flow_path)

                viz_warp_imgs_path = './viz_warp_imgs/' + data_name
                if not os.path.exists(viz_warp_imgs_path):
                    os.makedirs(viz_warp_imgs_path)
                ...
                plt.savefig(viz_flow_path + '/%02d.jpg'%i)
                plt.close()

                warped_im2 = warp_flow(img2, fwd_flow)
                warped_im0 = warp_flow(img1, bwd_flow)
  
                cv2.imwrite(viz_warp_imgs_path + '/im_%05d.jpg'%(i), img1[..., ::-1])
                cv2.imwrite(viz_warp_imgs_path + '/im_%05d_fwd.jpg'%(i), warped_im2[..., ::-1])
                cv2.imwrite(viz_warp_imgs_path + '/im_%05d_bwd.jpg'%(i + 1), warped_im0[..., ::-1])
