...
def run(basedir, 
    ...
    for ind in range(len(img_names)):
        ...
        if VIZ:
            midas_output_path = './midas_otuputs/' + data_name
            if not os.path.exists(midas_output_path):
                os.makedirs(midas_output_path)
            ...
            plt.savefig(midas_output_path + '/%s'%(img_name.split('/')[-1]))
            plt.close()
...
if __name__ == "__main__":
    ...
    run(BASE_DIR, INPUT_PATH, 
        OUTPUT_PATH, MODEL_PATH, DATA_NAME, 
        args.resize_height)
