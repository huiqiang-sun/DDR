...
def evaluation():
    ...
    poses_temp = poses.clone().detach()

    if args.learn_poses:
        num_img = poses.shape[0]
        pose_param_net = LearnPose(num_img, None).to(device)
    ...
    with torch.no_grad():
        ...
        for img_i in i_train:
            ...
            for camera_i in range(0, 12):
                ...
                #     continue
                if idx[img_i] - 1 == camera_i:
                    continue

                c2w = poses_temp[camera_i]
                # if args.learn_poses:
                    # if camera_i + 1 == 5:
                    #     c2w = pose_param_net(0)
                    # elif camera_i + 1 == 6:
                    #     c2w = pose_param_net(8)
                    # elif camera_i + 1 == 7:
                    #     c2w = pose_param_net(16)
                    # c2w = pose_param_net(camera_i)
                ...
                filename = os.path.join(save_path, 'rgb_{}_{}.jpg'.format(img_i, camera_i))
                imageio.imwrite(filename, rgb)
                filename = os.path.join(save_path, 'rgb_{}_{}_gt.jpg'.format(img_i, camera_i))
        ...
        file = open(os.path.join(basedir, expname, 'result.txt'),'w')
        file.write('mean_psnr {}\n'.format(mean_psnr))
        file.write('mean_ssim {}\n'.format(mean_ssim))
        file.write('mean_lpips {}\n'.format(mean_lpips))
        file.write('mean_psnr dy {}\n'.format(mean_psnr_dy))
        file.write('mean_ssim dy {}\n'.format(mean_ssim_dy))
        file.write('mean_lpips dy {}\n'.format(mean_lpips_dy))
        file.close
