...
def _load_data(basedir, start_frame, end_frame, 
    ...
    #进行图像缩放，具体缩放过程的代码可以见DynamicNeRF
    sfx = ''
    ...
    #poses中最后一列前两个数值为图像大小，最后一个数值为原数值乘以缩放因子
    ...
    #将图像数值转换为0-1范围，imgs大小为[H,W,C,N]，为512*288的图像
    ...
    # print('aaa', masks.shape)
    # sys.exit()

    #motion_coords是一个列表，其中有N项（N为图像总数量）
    #每一项的大小为[M,2]，他代表每张图像的M个坐标点，这些点是mask>0.1的点
    motion_coords = []
...
def _load_data_immerse(basedir, start_frame, end_frame, 
               factor=None, width=None, height=None, 
               load_imgs=True, evaluation=False):
    print('factor ', factor)
    print('height ', height)
...
# 原来的poses作用是将图片所在的相机坐标系转换为世界坐标系，然而世界坐标系是随意规定的，
# 因此每张图片转换后的世界坐标系可能不同，这会使得之后的渲染出现问题
# 因此该函数将poses进行变换，变换后的poses的作用是将图片所在的相机坐标系转换成一个类似于平均的相机坐标系，
# 这样当把坐标系转换到这个平均的相机坐标系后，在使用ndc坐标系时不同角度发射的光线在相同点就会像相交。
def recenter_poses(poses):

    poses_ = poses+0
    bottom = np.reshape([0,0,0,1.], [1,4])
    c2w = poses_avg(poses)   #c2w大小[3,5]，其中最后一列为hwf
    c2w = np.concatenate([c2w[:3,:4], bottom], -2)  #大小变为[4,4]
...
def load_llff_data(basedir, start_frame, end_frame, 
    ...
    # Correct rotation matrix ordering and move variable dim to axis 0
    # 这一部分得到正确的旋转矩阵，同时将几个变量的N所在的维度调整到第一维
    # poses大小[N,3,5]，bds大小[N,2]，images大小[N,W,H,C]，disp大小[N,W,H]，masks大小[N,W,H]
    poses = np.concatenate([poses[:, 1:2, :], 
                            -poses[:, 0:1, :], 
                            poses[:, 2:, :]], 1)
    poses = np.moveaxis(poses, -1, 0).astype(np.float32)
    ...
    # 对poses进行调整变换，变换后大小不变，依旧为[N,3,5]
    if recenter:
        poses = recenter_poses(poses)
        
    #c2w是在target_idx那一帧的旋转矩阵，大小为[3,5]
    c2w = poses[target_idx, :, :]
    # c2w = poses_avg(poses)
    ...
    # Generate poses for spiral path
    # render_poses = render_path_spiral(c2w_path, up, rads, focal, zdelta, zrate=.5, rots=N_rots, N=N_views)
    # render_poses为一个长度为60的列表，每一个元素的大小为[3,5]，表示一个角度的相机参数。大小为[60,3,5]
    render_poses = render_wander_path(c2w)
    render_poses = np.array(render_poses).astype(np.float32)
    ...
    return images, disp, masks, poses, bds, render_poses, c2w, motion_coords
...
def render_wander_path(c2w, hwf=None):
    if hwf is None:
        hwf = c2w[:,4:5]
    num_frames = 60
    max_disp = 64.0 # 64 , 48
    ...
    for i in range(num_frames):
        ...
        #i_pose是一个4*4的矩阵，其中最后一行为[0，0，0，1]，最后一列前三个数值为三个trans
        i_pose = np.concatenate([
            np.concatenate(
                [np.eye(3), np.array([x_trans, y_trans, z_trans])[:, np.newaxis]], axis=1),
            np.array([0.0, 0.0, 0.0, 1.0])[np.newaxis, :]
