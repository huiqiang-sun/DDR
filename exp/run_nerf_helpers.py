...
# Ray helpers
# 该函数根据输入的图像信息来生成世界坐标系下的每个像素点的光线信息，返回o和d,大小为[H,W,3]
# 输入H，W为图像的高和宽，focal为焦距，c2w为图像对应的相机外参，包括旋转矩阵和平移矩阵
# 先得到图像平面的像素坐标，在通过相机内参得到相机坐标系下的方向，最后通过相机外参得到世界坐标系下的方向
def get_rays(H, W, focal, c2w):
    # i，j最终都是H行，W列的二维张量，用来表征图像的索引，其中i表示列，j表示行。
    # 比如i=3,j=5就表示原图像第5行第3列的像素点
    i, j = torch.meshgrid(torch.linspace(0, W-1, W), torch.linspace(0, H-1, H))  # pytorch's meshgrid has indexing='ij'
    i = i.t()
    j = j.t()
    # dirs是一个[H,W,3]的三维张量，它表示图像中每个像素点的方向信息
    # 其中[H,W,0]为横向偏移角的正弦值，代表水平方向偏移（向右偏移为负）；
    # [H,W,1]为纵向偏移角的正弦值，代表垂直方向偏移（向上偏移为正）；
    # [H,W,2]为-1，表示z方向
    dirs = torch.stack([(i-W*.5)/focal, -(j-H*.5)/focal, -torch.ones_like(i)], -1)
    # Rotate ray directions from camera frame to the world frame
    # 利用相机外参将光线方向旋转到世界坐标系
    # rays_d和rays_o大小都为[H,W,3]
...
# 该函数的作用是将世界坐标系的光线坐标转换为ndc坐标系下的光线坐标
# 输出的rays_d和rays_o大小都为[H,W,3]
...
# 该函数用来计算预测深度pred_depth以及真实深度gt_depth之间的误差损失
# 先将两个深度图归一化到均值为0，方差为1，在计算两个归一化后的深度的均方差损失
...
# 该函数用来计算预测图像pred以及真实图像gt之间的均方误差MSE
# mask表示遮挡权重，若图像某点在两个时间点之间有遮挡或反遮挡现象，则该点遮挡权重为0
...
# 该函数用来计算预测值pred以及真实值gt之间的平均绝对误差MAE
# mask表示遮挡权重
...
# 将相机坐标系下的3d点pts_3d通过相机内参转换为图像上的2d点pts_2d
# 输出的pts_2d大小为[...,2]
...
# 该函数的作用是将平均相机坐标系下的坐标pts_ref转换为相机坐标系下的坐标pts_prev
# 其中raw_rot_ref2prev和raw_trans_ref2prev分别代表对应的旋转矩阵和平移矩阵
...
# 该函数是将ndc坐标系下的坐标转换为平均相机坐标系下的坐标（注意z的方向与相机射线方向相反）
...
# 该函数的作用是通过动态模型的空间点渲染权重以及空间点坐标进行加权求和
# 使得每条光线都得到一个加权求和后的空间坐标点，再将这个点投影到图像平面
# 输入c2w为相机外参，weights_ref为动态渲染空间点权重，raw_pts为空间点坐标
...
# 该函数通过前一帧和后一帧图像的相机参数，得到动态部分当前帧的点对应在前后两帧的哪个地方
# 该函数输入pose_post、pose_ref、pose_prev大小均为[3,4]，分别表示下一帧、当前帧和上一帧图像的外参
# H, W, focal分别表示图像高、宽和相机焦距；ret中存放渲染结果
# 输出pts_2d_post和pts_2d_prev表示当前帧的动态点对应在下一帧和上一帧的哪个地方
...
# 该函数输出给定图像帧对应的gt的flow以及mask
# 输入basedir为存储数据的路径，img_i表示图像序号，start_frame表示第一帧图像的序号
# fwd为True表示加载后向的数据；为False表示加载前向的数据
...
# 该函数用来计算场景流的空间平滑约束
# pts_1_ndc和pts_2_ndc分别表示当前时刻和某一相邻时刻的坐标，最后使用平均绝对误差进行计算
...
# 该函数用来计算场景流的时间平滑约束
# pts_ref_ndc, pts_post_ndc, pts_prev_ndc分别表示当前时刻、下一时刻以及上一时刻的坐标
# 最后使用均方误差进行计算
