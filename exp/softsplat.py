...
def cupy_kernel(strFunction, objVariables):
    ...
    # 将strFunction指向的代码中的SIZE_3(output)替换为W；SIZE_2(output)替换为H
    # SIZE_1(output)替换为5；SIZE_0(output)替换为1
	while True:
		...
		intArg = int(objMatch.group(2))  # 3/2/1/0

		strTensor = objMatch.group(4)  # 'output'
		intSizes = objVariables[strTensor].size()  # [1,5,H,W]
	...
	while True:
		...
		intArgs = int(objMatch.group(2))  # 4
		strArgs = objMatch.group(4).split(',')

		strTensor = strArgs[0]  # 'output'
		intStrides = objVariables[strTensor].stride()  # [5*H*W, H*W, W, 1]
